...
{
    QString filename = QFileDialog::getSaveFileName(this, QStringLiteral("另存为"), QStringLiteral("新建OFD文档.ofd"), QStringLiteral("OFD 文档"));
    if(filename.isEmpty())
        return;
...
bool RichTextProcess:: saveToRichText()
{
    int i=0;
    //获得文件名
    QString filename = QFileDialog::getSaveFileName(this, QStringLiteral("待编辑文件"), QStringLiteral("新建OFDX文件.ofdx"), QStringLiteral("OFDX 文档"));
...
bool RichTextProcess::openRichText()
{
    ...
    QString filename = QFileDialog::getOpenFileName(this, QStringLiteral("选择OFDX文档"), ".", tr("OFDX (*.ofdx *.ofd)"));
    if(filename.isEmpty())
       return false;
	...
	file1.write(reader.fileData(QString::fromLocal8Bit("%1").arg(filename)));//解压文件
	file1.close();
	reader.close();*/
...
QMessageBox* RichTextProcess::showMessage()
{
    ...
    msgBox->setText(QStringLiteral("编辑区内有内容"));
    msgBox->setInformativeText(QStringLiteral("是否保存未OFDX文档?"));
    msgBox->setStandardButtons( QMessageBox::Save | QMessageBox::Discard | QMessageBox::Cancel);
    msgBox->setButtonText (QMessageBox::Save, QStringLiteral("保 存"));
    msgBox->setButtonText (QMessageBox::Discard, QStringLiteral("不保存"));
    msgBox->setButtonText (QMessageBox::Cancel, QStringLiteral("取 消"));
    return msgBox;
}
